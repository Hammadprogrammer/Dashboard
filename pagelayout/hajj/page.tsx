...
export default function HajjDashboardPage() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === "price") {
        // Validation for price input: allows numbers and up to two decimal places
        if (/^\d*(\.\d{0,2})?$/.test(value) || value === "") {
            setFormState(prev => ({ ...prev, [name]: value }));
        }
    } else if (name === "category") {
        setCategory(value as Package["category"]);
    } else {
        setFormState(prev => ({ ...prev, [name]: value }));
    }
  };
  ...
  const handleEdit = (pkg: Package) => {
    ...
    setFormState({ title: pkg.title, price: pkg.price.toString() });
  ...
  const fetchPackages = async () => {
    try {
      setIsLoading(true);
      const res = await fetch("/api/hajj"); // Changed to the clean endpoint
      if (!res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ************ CRITICAL CHANGE: REMOVED CLIENT-SIDE DELETE LOGIC ************
  ...
  const isAnyActionDisabled = isProcessing || isLoading || !!id;
  const { title, price } = formState;
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto mt-8 md:mt-12">
      ...
      {/* Loading/Processing Overlay (Unified Loader) */}
      ...
      >
        <h2 className="text-xl font-bold text-yellow-400 flex items-center">
          {id ? "Edit Hajj Package" : "Add New Hajj Package"}
          ...
          {packages.map((pkg) => (
            ...
            >
              ...
              <p className="text-gray-400 mb-4 text-sm font-semibold">
                Category: <span className="text-yellow-300">{pkg.category}</span>
              </p>
              ...
              <div className="flex justify-between gap-2 mt-auto pt-3 border-t border-gray-700">
                ...
                <div className="flex gap-2">
                  <Link href="#hajj-heading" passHref>
                    <button
      ...
      {/* --- MODAL: Status Message (Consistent Style) --- */}
      ...
      {/* --- MODAL: Delete Confirmation (Consistent Style) --- */}
