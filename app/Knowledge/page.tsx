...
export default function KnowledgeDashboard() {
  ...
  const fetchItems = async () => {
    try {
      ...
      showModal(`⚠️ Error fetching knowledge items: ${error.message}`, "error");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title.trim() && !description.trim() && !file && !editingId) {
      return showModal(
        "⚠️ Please provide a title, description, and a file for a new item.",
        "warning"
      );
    }
    ...
    try {
      ...
      showModal(`⚠️ Error saving file: ${error.message}`, "error");
    } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showModal(`⚠️ Could not delete file: ${error.message}`, "error");
    } finally {
      setIsProcessing(false);
  ...
  const handleDownload = async (url: string, title: string, id: number) => {
    try {
      ...
      // Sanitize title for filename and ensure .pdf extension
      ...
      showModal(`⚠️ Download failed: ${error.message}`, "error");
    }
  };
